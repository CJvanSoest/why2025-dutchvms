...
static i2c_device_t *_i2c_device_create(void *dev, uint8_t address, uint32_t clk_speed) {
    ...
    if (device->devices[address]) {
        ESP_LOGW(TAG, "Device %i already opened", address);
        return NULL;
    }
...
device_t *badgevms_i2c_bus_create(char const *name, uint8_t port, uint32_t clk_speed) {
    ...
    /* On main-bus init, bring up the LED-matrix add-on (see board_bringup.c):
     * detect the PCA9698 on I2C2 (GPIO22/9), then run the matrix refresh +
     * WS2812 status-LED tasks. */
    if (port == 0) {
        board_bringup_start();
    }
