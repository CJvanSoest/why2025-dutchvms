...
static bool checkreturn
    default_extension_decoder(pb_istream_t *stream, pb_extension_t *extension, uint32_t tag, pb_wire_type_t wire_type);
static bool checkreturn
            decode_extension(pb_istream_t *stream, uint32_t tag, pb_wire_type_t wire_type, pb_extension_t *extension);
static bool pb_field_set_to_default(pb_field_iter_t *field);
static bool pb_message_set_to_defaults(pb_field_iter_t *iter);
static bool checkreturn pb_dec_bool(pb_istream_t *stream, pb_field_iter_t const *field);
static bool checkreturn pb_dec_varint(pb_istream_t *stream, pb_field_iter_t const *field);
static bool checkreturn pb_dec_bytes(pb_istream_t *stream, pb_field_iter_t const *field);
static bool checkreturn pb_dec_string(pb_istream_t *stream, pb_field_iter_t const *field);
static bool checkreturn pb_dec_submessage(pb_istream_t *stream, pb_field_iter_t const *field);
static bool checkreturn pb_dec_fixed_length_bytes(pb_istream_t *stream, pb_field_iter_t const *field);
...
static bool checkreturn buf_read(pb_istream_t *stream, pb_byte_t *buf, size_t count) {
    pb_byte_t const *source = (pb_byte_t const *)stream->state;
...
static bool checkreturn pb_readbyte(pb_istream_t *stream, pb_byte_t *buf) {
    ...
    if (!stream->callback(stream, buf, 1))
        PB_RETURN_ERROR(stream, "io error");
#else
    *buf          = *(pb_byte_t const *)stream->state;
...
static bool checkreturn read_raw_value(pb_istream_t *stream, pb_wire_type_t wire_type, pb_byte_t *buf, size_t *size) {
    ...
    switch (wire_type) {
        ...
        case PB_WT_64BIT: *size = 8; return pb_read(stream, buf, 8);

        case PB_WT_32BIT: *size = 4; return pb_read(stream, buf, 4);
...
static bool checkreturn decode_static_field(pb_istream_t *stream, pb_wire_type_t wire_type, pb_field_iter_t *field) {
    switch (PB_HTYPE(field->type)) {
        case PB_HTYPE_REQUIRED: return decode_basic_field(stream, wire_type, field);
...
static bool checkreturn allocate_field(pb_istream_t *stream, void *pData, size_t data_size, size_t array_size) {
     ...
     */
    {
        size_t const check_limit = (size_t)1 << (sizeof(size_t) * 4);
...
static bool checkreturn decode_field(pb_istream_t *stream, pb_wire_type_t wire_type, pb_field_iter_t *field) {
    ...
    switch (PB_ATYPE(field->type)) {
        case PB_ATYPE_STATIC: return decode_static_field(stream, wire_type, field);

        case PB_ATYPE_POINTER: return decode_pointer_field(stream, wire_type, field);

        case PB_ATYPE_CALLBACK: return decode_callback_field(stream, wire_type, field);
    ...
    if (PB_LTYPE(type) == PB_LTYPE_EXTENSION) {
        ...
        if (init_data) {
            if (PB_LTYPE_IS_SUBMSG(field->type) &&
                (field->submsg_desc->default_value != NULL || field->submsg_desc->field_callback != NULL ||
                 field->submsg_desc->submsg_info[0] != NULL)) {
...
static bool checkreturn
    pb_decode_inner(pb_istream_t *stream, pb_msgdesc_t const *fields, void *dest_struct, unsigned int flags) {
    ...
    uint32_t const   allbits     = ~(uint32_t)0;
...
bool checkreturn pb_decode_ex(pb_istream_t *stream, pb_msgdesc_t const *fields, void *dest_struct, unsigned int flags) {
    bool status;
...
bool checkreturn pb_decode(pb_istream_t *stream, pb_msgdesc_t const *fields, void *dest_struct) {
...
static bool checkreturn pb_dec_string(pb_istream_t *stream, pb_field_iter_t const *field) {
    ...
    if (!pb_validate_utf8((char const *)dest))
