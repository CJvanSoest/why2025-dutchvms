...
#if H_HOST_RESTART_NO_COMMUNICATION_WITH_SLAVE && H_HOST_RESTART_NO_COMMUNICATION_WITH_SLAVE_TIMEOUT_MS != -1
		/* CJ-DEBUG-BLE fix (task #23 follow-up): the init_timeout_timer
		 * armed above expects the slave to send a fresh init event, but
		 * if transport is ALREADY up (e.g. WiFi already brought it up
		 * earlier, and this reconfigure call is from BT enabling later),
		 * the slave has no reason to send another one -- the timer fires
		 * 5s later regardless and force-resets the host even though
		 * everything is working. Stop it here, matching what the real
		 * init-event path implicitly does via set_transport_state(). */
		if (init_timeout_timer) {
			g_h.funcs->_h_timer_stop(init_timeout_timer);
			init_timeout_timer = NULL;
			ESP_LOGI(TAG, "Transport already up -- stopped init timeout timer");
		}
#endif
	}
